// ECHO agent registry — the single source of truth for the agent roster, used by
// both the /architecture page (so judges can see it live, not just read about it)
// and docs/ON_DEVICE_ARCHITECTURE.md. Update this file first when an agent's status
// changes; the UI and docs should stay downstream of it, not drift independently.
...
export const AGENTS: AgentSpec[] = [
  {
    ...
    status: "built",
    deterministic: true,
    round1: "Keyword-based classifyIntent() in src/lib/agents/routerAgent.ts",
    round2: "Unchanged — stays deterministic to avoid spending an inference pass on tool selection",
  ...
  },
  {
    ...
    status: "built",
    deterministic: true,
    round1: "twin.ts + tools.ts, pure arithmetic over mock monthly snapshots",
    round2: "Unchanged — this is a math task, not a language task; no model involved in either round",
  ...
  },
  {
    ...
    status: "built",
    deterministic: true,
    round1: "get_evidence() in tools.ts",
    round2: "Unchanged retrieval logic; the on-device Synthesis Agent is what reads its output",
  ...
  },
  {
    ...
    responsibilities: [
      ...
      "Provide the match target for the Vision Agent's photo recognition",
    ],
    status: "built",
    deterministic: true,
    round1: "query_memory_graph() in tools.ts, static in-memory node/edge arrays",
    round2: "Same traversal logic; storage moves to an on-device store once data is dynamic, not static mock data",
  ...
  },
  {
    ...
    status: "built",
    deterministic: true,
    round1: "simulate_scenario() in tools.ts, transparent heuristic formula",
    round2: "Stays deterministic on purpose — a caregiver-facing simulation should not silently hallucinate an outcome",
  ...
  },
  {
    id: "speech",
    name: "Speech Agent",
    role: "Analyzes acoustic patterns in an uploaded voice recording",
    responsibilities: [
      "Real energy-based voice-activity detection (pause ratio, activity rate)",
      "Round 2: real transcription + understanding via a multimodal on-device model",
    ],
    status: "partial",
    deterministic: true,
    round1: "analyzeSpeechFile() — genuine Web Audio API DSP, verified against synthetic test audio",
    round2: "Gemma 3n's native audio input for real transcription; the DSP stays as a fast pre-filter ahead of the model call",
  ...
  },
  {
    ...
    status: "planned",
    deterministic: false,
    round1: "Not implemented — typed stub only (visionAgent.matchPhoto throws on purpose)",
    round2: "Gemma 3n native image input, matched against Memory Agent nodes via on-device embedding comparison",
  ...
  },
  {
    ...
    status: "built",
    deterministic: true,
    round1: "detectMeaningfulChanges() + a real browser Notification, user-gesture triggered from the dashboard bell",
    round2: "Same detection logic; the notification becomes a native Android notification from a background check",
  ...
  },
  {
    ...
    status: "partial",
    deterministic: false,
    round1: "investigate() in gemini.ts — cloud Gemini if GEMINI_API_KEY is set, else a deterministic template",
    round2: "THE swap point — Gemma 3n (E2B, INT4) via MediaPipe LLM Inference API, on-device. See android-reference/.",
